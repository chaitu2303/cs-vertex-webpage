...
export default async function LoaderDemoPage() {
  const [services, projects, team, announcements, testimonials, posters] = await Promise.all([
    prisma.service.findMany({ where: { published: true }, orderBy: { order: 'asc' } }),
    prisma.project.findMany({ where: { published: true, isFeatured: true }, orderBy: { order: 'asc' } }),
    ...
    prisma.poster.findMany({ where: { published: true, showOnHome: true }, orderBy: { displayOrder: 'asc' } })
  ])

...
export default function TeamClient({ initialTeam }: { initialTeam: any[] }) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setIsAdding(false)
    setEditingId(null)
    setFormData({ name: '', role: '', email: '', bio: '', linkedinUrl: '', githubUrl: '' })
    setLoading(false)
  }
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
        ...
        <button 
          onClick={() => { setIsAdding(!isAdding); setEditingId(null); setFormData({ name: '', role: '', email: '', bio: '', linkedinUrl: '', githubUrl: '' }) }}
            ...
            <div className="premium-glass-panel" style={{ padding: '24px' }}>
              ...
              <form onSubmit={handleSave} style={{ display: 'grid', gap: '16px', gridTemplateColumns: '1fr 1fr' }}>
                ...
                <div>
                  ...
                  <input type="url" value={formData.linkedinUrl} onChange={e => setFormData({...formData, linkedinUrl: e.target.value})} className="premium-input" placeholder="https://linkedin.com/in/username" />
                </div>
                <div>
                  <label className="premium-label">GitHub URL</label>
                  <input type="url" value={formData.githubUrl} onChange={e => setFormData({...formData, githubUrl: e.target.value})} className="premium-input" placeholder="https://github.com/username" />
                </div>

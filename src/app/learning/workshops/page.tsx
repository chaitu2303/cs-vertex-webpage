...
export default async function WorkshopsPage() {
  const workshops = await readFromJSON('workshops', () => prisma.workshop.findMany({ 
            ...
            <div key={w.id} style={{ background: '#111', border: '1px solid #222', borderRadius: '12px', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
              ...
              <div style={{ padding: '24px', flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '20px', fontSize: '13px' }}>
                  <div style={{ background: '#000', padding: '10px', borderRadius: '6px', border: '1px solid #333' }}><span style={{ color: '#666', display: 'block', fontSize: '11px', textTransform: 'uppercase' }}>Date</span>{w.date ? new Date(w.date).toLocaleDateString() : 'TBA'}</div>
                  <div style={{ background: '#000', padding: '10px', borderRadius: '6px', border: '1px solid #333' }}><span style={{ color: '#666', display: 'block', fontSize: '11px', textTransform: 'uppercase' }}>Time</span>{w.time || 'TBA'}</div>

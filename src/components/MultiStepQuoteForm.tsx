  ...
  if (status === 'success') {
    return (
      <div style={{ background: 'rgba(255, 92, 42, 0.05)', padding: '50px', borderRadius: '16px', textAlign: 'center', border: '1px solid var(--acid)', backdropFilter: 'blur(10px)', maxWidth: '700px', margin: '0 auto' }}>
        <div style={{ fontSize: '48px', color: 'var(--acid)', marginBottom: '20px' }}>✓</div>
          ...
          {step === 1 && (
            <div className="fade-in">
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '15px' }}>
                {[
                  { id: 'Software Engineering', title: 'Software Engineering', desc: 'Web, Mobile, SaaS, ERP', icon: '💻' },
                  { id: 'AI Solutions', title: 'AI Solutions', desc: 'ML, NLP, Computer Vision', icon: '🧠' },
                  { id: 'IoT / Embedded', title: 'IoT / Embedded', desc: 'Hardware, Firmware, Sensors', icon: '🔌' },
                  { id: 'Robotics', title: 'Robotics', desc: 'Autonomous Systems, Automation', icon: '🤖' },
                  { id: 'Cloud / DevOps', title: 'Cloud / DevOps', desc: 'AWS/GCP/Azure, CI/CD, Scaling', icon: '☁️' },
                ].map(svc => (
                  <div 
                    key={svc.id}
      ...
      {showAuthModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(10px)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ background: '#111', border: '1px solid #333', padding: '40px', borderRadius: '16px', maxWidth: '400px', width: '100%', position: 'relative' }}>
            <button onClick={() => setShowAuthModal(false)} style={{ position: 'absolute', top: '15px', right: '15px', background: 'none', border: 'none', color: '#F5F1EA', cursor: 'pointer', fontSize: '18px' }}>✕</button>

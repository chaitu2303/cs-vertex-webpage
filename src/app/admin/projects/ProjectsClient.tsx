...
export default function ProjectsClient({ initialProjects }: { initialProjects: any[] }) {
  ...
  const [formData, setFormData] = useState({ 
    ...
    github: '', liveDemo: '', documentation: '', isFeatured: false
  })
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    Object.entries(formData).forEach(([key, value]) => fd.append(key, typeof value === 'boolean' ? value.toString() : value))
    ...
    setIsAdding(false)
    setEditingId(null)
    setFormData({ title: '', category: '', shortSummary: '', technologies: '', challenge: '', solution: '', impact: '', github: '', liveDemo: '', documentation: '', isFeatured: false })
    setLoading(false)
  }
  ...
  const handleEdit = (proj: any) => {
    setFormData({ 
      ...
      github: proj.github || '', liveDemo: proj.liveDemo || '', documentation: proj.documentation || '', isFeatured: proj.isFeatured || false
    })
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
        ...
        <button 
          onClick={() => { setIsAdding(!isAdding); setEditingId(null); setFormData({ title: '', category: '', shortSummary: '', technologies: '', challenge: '', solution: '', impact: '', github: '', liveDemo: '', documentation: '', isFeatured: false }) }}
            ...
            <div className="premium-glass-panel" style={{ padding: '24px' }}>
              ...
              <form onSubmit={handleSave} style={{ display: 'grid', gap: '16px', gridTemplateColumns: '1fr 1fr' }}>
                ...
                <div>
                  ...
                  <input type="text" value={formData.github} onChange={e => setFormData({...formData, github: e.target.value})} className="premium-input" placeholder="https://github.com/..." />
                </div>
                <div>
                  <label className="premium-label">Live Demo URL</label>
                  <input type="text" value={formData.liveDemo} onChange={e => setFormData({...formData, liveDemo: e.target.value})} className="premium-input" placeholder="https://..." />
                </div>
                <div>
                  <label className="premium-label">Documentation URL</label>
                  <input type="text" value={formData.documentation} onChange={e => setFormData({...formData, documentation: e.target.value})} className="premium-input" placeholder="https://docs..." />
                </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  ...
                  <label htmlFor="isFeatured" className="premium-label" style={{ margin: 0 }}>Featured Project</label>
                </div>

...
import { Pin, Download, ArrowRight } from 'lucide-react'
      ...
      <div className="board-main" style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
        {pinned.map(notice => (
          <div key={notice.id} className="notice-card pinned" style={{ background: 'rgba(255, 92, 42, 0.05)', border: '1px solid var(--acid)', padding: '30px', borderRadius: '8px' }}>
            ...
            {notice.fileUrl && (
              <a href={notice.fileUrl} download className="download-btn group" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', marginTop: '20px', padding: '10px 20px', border: '1px solid #444', color: '#fff', textDecoration: 'none', fontSize: '13px', borderRadius: '4px', transition: '0.2s' }}>
                <Download size={14} strokeWidth={1.5} className="hover-orange-icon" /> Download Attachment
              </a>
            )}
          </div>
        ))}
        ...
        {regular.map(notice => (
          <div key={notice.id} className="notice-card" style={{ borderBottom: '1px solid #222', padding: '20px 0' }}>
            ...
            {notice.fileUrl && (
              <a href={notice.fileUrl} download className="group" style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', marginTop: '15px', color: 'var(--acid)', textDecoration: 'none', fontSize: '13px' }}>
                Download Attachment <ArrowRight size={14} strokeWidth={1.5} className="hover-orange-icon" />
              </a>
            )}
          </div>
        ))}

...
export function Newsletter({ actionUrl }: { actionUrl: string }) {
  ...
  if (status === 'success') {
    return (
      <p style={{ color: 'var(--acid)', fontSize: '13px', margin: '20px 0 0', display: 'flex', alignItems: 'center', gap: '8px' }}>
        <CheckCircle size={16} strokeWidth={2} /> Subscribed successfully. Welcome to the CS Vertex loop.
      </p>
    )
  }

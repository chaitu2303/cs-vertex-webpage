...
export default function EditCareerPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    fetch('/api/admin/careers')
      .then(res => res.json())
      .then(data => {
        const career = data.find((c: any) => c.id === params.id)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    await fetch(`/api/admin/careers/${params.id}`, {
      method: 'PUT',
  ...
  if (!formData) return <div className="text-[#888]">Loading...</div>

  return (
    <div className="max-w-2xl">
      <h1 className="text-2xl font-semibold tracking-tight text-[#ededed] mb-8">Edit Position</h1>

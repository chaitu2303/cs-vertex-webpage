...
/* ─────────────────────────────────────────────────────────
   ACT 4 — ENERGY CORE
   ───────────────────────────────────────────────────────── */
function EnergyCore({ coreScale }: { coreScale: number }) {
  const ref = useRef<THREE.Mesh>(null!)

  useFrame((state) => {
    if (!ref.current) return
    const time = state.clock.getElapsedTime()
    const pulse = 1.0 + Math.sin(time * 18.0) * 0.12
    ref.current.scale.setScalar(coreScale * 2.8 * pulse)
    
    const mat = ref.current.material as THREE.MeshBasicMaterial
    mat.opacity = coreScale * (0.6 + Math.random() * 0.4)
  })
...
/* ─────────────────────────────────────────────────────────
   ACT 4 — CORE EXPLOSION PARTICLES
   ───────────────────────────────────────────────────────── */
function ExplosionParticles({ phase }: { phase: string }) {
  const ref = useRef<THREE.Points>(null!)
  const N = 240
  ...
  const { positions, velocities, colors } = useMemo(() => {
    ...
    const vel = new Float32Array(N * 3)
    const col = new Float32Array(N * 3)
    const colorsList = [new THREE.Color('#E8440A'), new THREE.Color('#FF9800'), new THREE.Color('#ffffff')]
  ...
  useFrame((_, dt) => {
    ...
    const active = ['EXPLODE', 'GATEWAY', 'ASSEMBLE'].includes(phase)
    if (active) {
      ageRef.current += dt
      const speedDecay = Math.max(0, 1 - ageRef.current * 0.7)
      
      for (let i = 0; i < N; i++) {
        arr[i*3]   += velocities[i*3]   * speedDecay
        arr[i*3+1] += velocities[i*3+1] * speedDecay
        arr[i*3+2] += velocities[i*3+2] * speedDecay
      }
  ...
  const show = ['EXPLODE', 'GATEWAY', 'ASSEMBLE'].includes(phase)
...
/* ─────────────────────────────────────────────────────────
   ACT 5 — DIGITAL ENERGY GATES
   ───────────────────────────────────────────────────────── */
function GateParticles({ gateOpen }: { gateOpen: number }) {
  const ref = useRef<THREE.Points>(null!)
  const N = 160
  ...
  useFrame((_, dt) => {
    ...
    for (let i = 0; i < N; i++) {
      const side = velocities[i*3]
      arr[i*3] = side * (0.55 + gateOpen * 6.5) + (Math.random() - 0.5) * 0.3
...
function EnergyGates({ gateOpen, phase }: { gateOpen: number; phase: string }) {
  const leftRef  = useRef<THREE.Mesh>(null!)
  const rightRef = useRef<THREE.Mesh>(null!)
  const show = ['MYSTERY', 'AWAKENING', 'RELEASE', 'LOGO_REVEAL'].includes(phase) && gateOpen < 0.995

  useFrame((state) => {
    if (!show) return
    const t = state.clock.getElapsedTime()
    const leftX = -0.55 - gateOpen * 6.5
    const rightX = 0.55 + gateOpen * 6.5
    
    if (leftRef.current) {
      leftRef.current.position.x = leftX
      leftRef.current.position.y = Math.sin(t * 2) * 0.1
      leftRef.current.scale.set(0.18, 10 + Math.sin(t * 12) * 0.4, 0.18)
    }
    if (rightRef.current) {
      rightRef.current.position.x = rightX
      rightRef.current.position.y = Math.sin(t * 2 + Math.PI) * 0.1
      rightRef.current.scale.set(0.18, 10 + Math.sin(t * 12 + Math.PI) * 0.4, 0.18)
    }
  })
  ...
  return (
    <group>
      {/* Left Plasma Column */}
      <mesh ref={leftRef} position={[-0.55, 0, 0]}>
        <cylinderGeometry args={[1, 1, 1, 8]}/>
        <meshBasicMaterial
          color="#E8440A"
          transparent
          opacity={0.8 - gateOpen * 0.7}
          wireframe
        />
      </mesh>
      {/* Right Plasma Column */}
      <mesh ref={rightRef} position={[0.55, 0, 0]}>
        <cylinderGeometry args={[1, 1, 1, 8]}/>
        <meshBasicMaterial
          color="#E8440A"
          transparent
      ...
      <GateParticles gateOpen={gateOpen}/>
    </group>
  )
}
...
function VelvetWrappedCloth({
  ...
  const show = ['MYSTERY', 'AWAKENING', 'RELEASE'].includes(phase)
...
/* ─────────────────────────────────────────────────────────
   HELIX FORGING SPARK PARTICLES
   ───────────────────────────────────────────────────────── */
function HelixSparks({ phase }: { phase: string }) {
  const ref = useRef<THREE.Points>(null!)
  const N = 120

  const { positions, angles, speeds } = useMemo(() => {
    const pos = new Float32Array(N * 3)
    ...
    return { positions: pos, angles: ang, speeds: spd }
  }, [])
  ...
  useFrame((state) => {
    ...
    const active = phase === 'RELEASE' || phase === 'LOGO_REVEAL'
    if (active) {
      for (let i = 0; i < N; i++) {
        const radius = 2.8 + Math.sin(time * 0.5 + i) * 0.7
        const theta = angles[i] + time * speeds[i] * 0.8
        const py = Math.sin(time * 0.8 + i * 0.2) * 2.2
  ...
  const show = phase === 'RELEASE' || phase === 'LOGO_REVEAL'
...
const sampleDigitPoints = (digit: string, count: number): THREE.Vector3[] => {
  if (typeof document === 'undefined') return Array(count).fill(0).map(() => new THREE.Vector3())
  
  const canvas = document.createElement('canvas')
  canvas.width = 64
  canvas.height = 64
  const ctx = canvas.getContext('2d')
  if (!ctx) return Array(count).fill(0).map(() => new THREE.Vector3())
  ...
  for (let i = 0; i < count; i++) {
    if (pixels.length === 0) {
      points.push(new THREE.Vector3((Math.random() - 0.5) * 4, (Math.random() - 0.5) * 4, 0))
    } else {
      const p = pixels[i % pixels.length]
...
function CountdownParticles({ value, phase }: { value: number; phase: string }) {
  const pointsRef = useRef<THREE.Points>(null!)
  const count = 1200
  
  const progress = useRef({ val: 1 })
  const currentPoints = useRef<THREE.Vector3[]>([])
  const targetPoints = useRef<THREE.Vector3[]>([])
  const lastValue = useRef(value)
  
  if (currentPoints.current.length === 0) {
    currentPoints.current = sampleDigitPoints(String(value), count)
    targetPoints.current = currentPoints.current
  }
  ...
  useEffect(() => {
    if (value !== lastValue.current) {
      if (pointsRef.current) {
        ...
        const arr = posAttr.array as Float32Array
        for (let i = 0; i < count; i++) {
          currentPoints.current[i] = new THREE.Vector3(arr[i*3], arr[i*3+1], arr[i*3+2])
        }
      }
      
      targetPoints.current = sampleDigitPoints(String(value), count)
      
      progress.current.val = 0
  ...
  useFrame((state) => {
    ...
    const posAttr = pointsRef.current.geometry.attributes.position as THREE.BufferAttribute
    const arr = posAttr.array as Float32Array
    const t = progress.current.val
    
    const dispersion = Math.sin(t * Math.PI) * 1.6
    
    for (let i = 0; i < count; i++) {
      const start = currentPoints.current[i]
      const end = targetPoints.current[i]
      
      const px = THREE.MathUtils.lerp(start.x, end.x, t)
      const py = THREE.MathUtils.lerp(start.y, end.y, t)
      const pz = THREE.MathUtils.lerp(start.z, end.z, t)
      
      const noiseX = (Math.sin(state.clock.getElapsedTime() * 10 + i) * 0.4 + (Math.random() - 0.5) * 0.6) * dispersion
      const noiseY = (Math.cos(state.clock.getElapsedTime() * 10 + i) * 0.4 + (Math.random() - 0.5) * 0.6) * dispersion
      const noiseZ = (Math.sin(state.clock.getElapsedTime() * 5 + i) * 0.4 + (Math.random() - 0.5) * 0.6) * dispersion
    ...
    posAttr.needsUpdate = true
    pointsRef.current.rotation.y = Math.sin(state.clock.getElapsedTime() * 0.3) * 0.15
    pointsRef.current.rotation.x = Math.cos(state.clock.getElapsedTime() * 0.2) * 0.08
  })
  ...
  const initialColors = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      let c = orange
      if (value <= 3) {
        c = Math.random() < 0.5 ? white : gold
      } else if (value <= 6) {
        c = Math.random() < 0.6 ? gold : orange
      }
      arr[i*3] = c.r
      arr[i*3+1] = c.g
      arr[i*3+2] = c.b
  ...
  const show = phase === 'FINAL_COUNTDOWN'
...
function Scene({
  ...
  const starCount = phase === 'CRITICAL' ? 6000 : tension > 0.5 ? 4500 : 2800
  ...
  return (
    <group ref={groupRef}>
      ...
      <HelixSparks phase={phase}/>

      <CountdownParticles value={countdownValue} phase={phase} />

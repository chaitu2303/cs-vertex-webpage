...
/* ═══════════════════════════════════════════════════════════════════════
   PHASE TYPE
   ═══════════════════════════════════════════════════════════════════════ */
type Phase = 'COUNTDOWN' | 'CURTAIN' | 'LOADING'
...
function CountdownScreen({ onComplete }: { onComplete: () => void }) {
  ...
  /* Countdown tick */
  useEffect(() => {
    if (time.total <= 0) { onComplete(); return }
    const id = setInterval(() => {
      const r = getRemaining(targetMsRef.current)
      setTime(r)
      if (r.total <= 0) { clearInterval(id); onComplete() }
    }, 1_000)
    return () => clearInterval(id)
  }, [onComplete, time.total])
...
export function LaunchGate({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const initial  = computeInitialPhase(targetMs)
    setPhase(initial)
    setMounted(true)
  }, [])
  ...
  const handleRevealComplete = useCallback(() => {
    ...
    // Always show loading screen after the reveal — clear the skip flag
    sessionStorage.removeItem('csvertex_preloader_done')
    goTo('LOADING')
  }, [goTo])

      ...
      <div className="testimonials-container">
        ...
        >
          {testimonials.length > 0 && (
            <div className="carousel-slide">
              <div className="testimonial-card">
                <div className="stars" style={{ display: 'flex', gap: '4px', marginBottom: '15px' }}>
                  {Array.from({ length: 5 }).map((_, idx) => (
                    <Star
                      key={idx}
                      size={16}
                      fill={idx < testimonials[currentIndex].rating ? 'var(--acid)' : 'transparent'}
                      color={idx < testimonials[currentIndex].rating ? 'var(--acid)' : '#444'}
                      strokeWidth={2}
                    />
                  ))}
                </div>
          ...
          <div className="carousel-controls">
            <button onClick={handlePrev} className="nav-btn" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <ChevronLeft size={18} />
            </button>
            <div className="dots">
            ...
            <button onClick={handleNext} className="nav-btn" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <div className="feedback-form-wrapper">
          ...
          {status === 'success' ? (
            <div className="success-msg" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '15px', color: 'var(--acid)', padding: '20px 0' }}>
              <CheckCircle size={32} strokeWidth={1.5} />
              <p style={{ color: '#fff', fontSize: '14px', margin: 0, textAlign: 'center' }}>Thank you! Your feedback has been submitted for review.</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="feedback-form">
              ...
              <div className="form-group rating-stars-group">
                ...
                <div className="stars-picker" style={{ display: 'flex', gap: '6px' }}>
                  {[1, 2, 3, 4, 5].map((star) => (
                    <button
                      key={star}
                      type="button"
                    ...
                    >
                      <Star
                        size={20}
                        fill={rating >= star ? 'var(--acid)' : 'transparent'}
                        color={rating >= star ? 'var(--acid)' : '#555'}
                        strokeWidth={2}
                      />
                    </button>
                  ))}
